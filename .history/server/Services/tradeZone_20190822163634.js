...
const methods = {
    ...
    const checkForNestedData = (aggregateSet, keyPair) => {
        console.log('length', Object.entries(keyPair).length, keyPair)
        if (Object.entries(keyPair).length > 1) {
            let keyPairs = Object.entries(data[i]).map(([key, value]) => ({key,value}))
            keyPairs.forEach(keyPair => checkForNestedData(aggregateSet, keyPair))
        } else {
            keyPairs.forEach(keyPair => aggregateSet[keyPair.key] += keyPair.value)
        }
    }
