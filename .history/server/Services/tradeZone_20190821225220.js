...
const methods = {
    ...
    const getZoneStats = async (points, callback) => {
        console.log('Balloon', points.length, points)
        let collection = []
        // for all points get stats then aggregate
        await getCitySDKStats('age', points[0].lat, points[0].lng).then(stats => console.log('XXXSTATSXX', stats))
    }
    ...
    const getCitySDKStats = (type, lat, lng) => {

        switch (type) {
            case 'social': 
                return citySDK.getSocial('tract', lat, lng, (data) => new Promise((res, rej) => res(data)))
            case 'income':
                citySDK.getIncome('tract', lat, lng, (data) => new Promise((res, rej) => res(data)))
                break;
            case 'age':
                citySDK.getAge('tract', lat, lng, (data) => new Promise((res, rej) => res(data)))
                break;
        }
    }
