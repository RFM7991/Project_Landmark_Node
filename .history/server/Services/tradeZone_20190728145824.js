...
const methods = {
    ...
    getZoneStats : (points, callback) => {
        ...
        points.forEach(e => {
            citySDK.getBasic('tract', e.lat, e.lng, (data) => {
                ...
                if (points.length == 0) 
                    methods.aggregateZoneStats(collection)
                  //  callback(JSON.stringify({collection : collection}))
            })
        })
    },
    ...
    aggregateZoneStats : (data) => {
        ...
        result.median_age = median_ageTotal / points.length
        result.median_income = median_incomeTotal / points.length
    }
}
