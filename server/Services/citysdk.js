...
var methods = {
  ...
  getCityPopulation: (lat, lng, callback) => {
    census({
      "vintage" : 2017,             
      "geoHierarchy" : {            
      ...
      "sourcePath" : ["acs", "acs5"],        
      "values" : 
      ["NAME", vars.POPULATION.TOTAL],
      "statsKey" : KEY 
    }, 
      (err, res) => {
          if (!err)
            callback(filter.filterCityPopulation(res))
      })
  },
 ...
 getBasic : function(range, lat, lng, callback) {
  var range = range
  if (range === 'tract') {
 census({
     "vintage" : 2017,             
   ...
   (err, res) => {
    callback(filter.filterBasic(res))
   }
 )
  } else if (range === 'zip code tabulation area') {
    census({
